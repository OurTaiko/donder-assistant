...
def calculate_result(arr):
    ...
    # 步骤3：计算体力密度A1和A2
    A1 = seg1_elements / sum1 * 1000 if sum1 > 0 else 0
    A2 = seg2_elements / sum2 * 1000 if sum2 > 0 else 0
    ...
    if rest_indices:
        ...
        # 修改B的计算公式，增加tanh(rest_sum/10000)项
        B = (len(rest_indices) / rest_sum * 1000) * (math.tanh(rest_sum / 10000))
    else:
        B = 0
    
    # 步骤5：计算加权平方平均值，使用新的权重分配
    result = (0.6 * Amax**2 + 0.3 * Amin**2 + 0.1 * B**2) ** 0.5
    
    return A1, A2, B, result
...
def main():
    ...
    try:
        A1, A2, B, result = calculate_result(arr)
        print(f"体力密度 A1: {A1:.2f}")
        print(f"体力密度 A2: {A2:.2f}")
        print(f"休息密度 B: {B:.2f}")
        print(f"Amax: {max(A1, A2):.2f}")
        print(f"Amin: {min(A1, A2):.2f}")
